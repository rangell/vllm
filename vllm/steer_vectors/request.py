...
class VectorConfig(
    ...
    Args:
        ...
        algorithm: Vector algorithm to use: 'direct' (default), 'erase', or 'loreft'
        normalize: Whether to normalize the vector (default: False, only applies to 'direct' algorithm)
        erase_intensity: Erasure intensity for 'erase' algorithm, scalar between 0.0 and 1.0 (default: 1.0)
    """
    path: str
    scale: float = 1.0
...
class SteerVectorRequest(
    ...
    Args:
        ...
        generate_after_k_tokens: Start applying from the k-th generated token onwards (skip first k tokens).
                                Mutually exclusive with generate_first_k_tokens.
        algorithm: Steer vector algorithm to use: 'direct' (default), 'erase', or 'loreft'
